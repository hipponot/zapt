...
module Zapt
  ...
  class ShellTask < FilesystemTask
    ...
    def command cmd, working_dir:nil, user:nil, host:nil, pem:nil, quiet:false, ignore_failure:false, dryrun:false
      rval, status = run_cmd cmd, working_dir, user, host, pem, quiet, dryrun, ignore_failure
    ...
    def run_cmd cmd, working_dir, user, host, pem, quiet, dryrun, ignore_failure
      cmd.insert(0,"cd #{working_dir};") if working_dir
      return Zapt.system(cmd, user, host, pem, quiet, ignore_failure) unless dryrun
      puts "dryrun: #{cmd}"
      return nil, 0
    end
